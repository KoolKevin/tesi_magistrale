...
int main() {
    ...
    start = clock();
    vectorized_mat_reduce_rows(a, res, M, N);
    end = clock();   
    double time_vectorized = ((double)(end-start) / CLOCKS_PER_SEC)*1000; // in ms
    printf("Tempo di esecuzione di vectorized_mat_reduce_rows: %.2fms\n", time_vectorized);
    printf("Speedup: %.2f\n", time_scalar/time_vectorized);

    print_vector(res, M);
    check_result(groundtruth, res, M);
    ...
    start = clock();
    autovectorized_mat_reduce_rows(a, res, M, N);
    end = clock();   
    double time_autovectorized = ((double)(end-start) / CLOCKS_PER_SEC)*1000; // in ms
    printf("Tempo di esecuzione di autovectorized_mat_reduce_rows: %.2fms\n", time_autovectorized);
    ...
    start = clock();
    vekt_mat_reduce_rows_wrapper(a, res, M, N);
    end = clock();   
    double time_vekt = ((double)(end-start) / CLOCKS_PER_SEC)*1000; // in ms
    printf("Tempo di esecuzione di vekt_mat_reduce_rows: %.2fms\n", time_vekt);
